...
**★ @태그만 쓰고, 그 대상의 외형은 본문에 다시 묘사하지 말 것.** 첨부하는 ref(캐릭터
`_face`/`_full`/`_voice`, 로케이션, prop, 직전 샷 영상 `@video`)는 본문에서 **그 `@태그`로만**
지칭한다. 그 대상의 **외형·의상·배경·내용을 본문에 또 묘사하면, 엔진이 첨부된 ref를 무시하고 그
묘사대로 새로 변형·생성**해 버린다(ref가 흔들림). `@태그` 옆엔 그 대상이 **무엇을 하는지(동작·배치·
표정·카메라)만** 적는다.
  - 예: ❌ `아이보리 커버를 씌운 시집 @p1_p_poetrybook_cover 를 펼쳐 든` → ✅ `@p1_p_poetrybook_cover 를 펼쳐 든`
  - 예: ❌ `교복 입은 @p1_c_sua_uniform_full 가` → ✅ `@p1_c_sua_uniform_full 가`
  - **startFrame/endFrame**도 같은 이유 — 그 프레임의 외형은 `startFrame.prompt`에만(그 한 장만
    뽑히게 간결히) 적고, shot 본문에는 그 프레임 내용을 재묘사하지 않는다(동작·연결만).
...
- **네이밍:** `{프로젝트}_{종류}_{고유이름}` · 구분자는 **`_`만**(하이픈·공백 불가).
  ...
  - **`_full`엔 `_face`를 동반한다(얼굴 일관성):** 인물이 식별 가능하게 나오는 근접~미디엄
    ...
    디렉터는 이 두 `@이름`을 신호로 **두 장(full+face)을 함께 엔진에 첨부**한다(도구는 엔진을
    호출하지 않으므로 첨부는 디렉터 몫).
    - 표기 예: `… @p1_c_jihoon_uniform_full(얼굴 일관성 위해 @p1_c_jihoon_uniform_face 동반) 이 …`
      또는 블로킹 끝에 ` · ref: @p1_c_jihoon_uniform_full + @p1_c_jihoon_uniform_face`.
    - **예외(동반 불필요):** 얼굴이 안 보이거나 의미 없는 경우 — 원경/실루엣/뒷모습, 손·발 등
      신체 부분 인서트, 군중 속 익명. 이때는 `_full`만(또는 ref 없이) 쓴다.
  - **대사가 있는 shot엔 화자의 `_voice`를 명시한다:** 본문에 따옴표 대사(말)가 있는 shot은,
    말하는 캐릭터의 `@..._voice`를 본문에 **별도로 명시**해 디렉터가 목소리 레퍼런스를 함께
    첨부하게 한다. 표기: shot 본문 끝에 ` · voice: @p1_c_jihoon_voice, @p1_c_sua_voice`
    (여러 화자면 나열, shot 단위로 한 번). 대사가 없는 shot(인서트·풍경·동작만)이나 목소리
    ref가 없는 단역(익명 행인 등)은 붙이지 않는다.
...
- **멀티컷**이면 컷마다 줄을 나누고 `Cut #N — ` 라벨로 시작한다(라벨과 내용은 `—`, 내부 요소는 `·`).
- **단일컷**이면 라벨 없이 위 포맷 한 줄로 쓴다.

### 예시 (한 Shot, 5컷)

```
Cut #1 — 와이드 설정샷 · 핸드헬드 팔로우 후 우측 패닝 · @p1_c_jihoon_full 이 빈 테이블 사이를 가로질러 오고 @p1_c_suah_full 이 일어선다 · 연기: 지훈 굳은 표정·긴장한 보폭, 수아 망설이다 천천히 기립
Cut #2 — 클로즈업 · 고정, 미세 드리프트 · @p1_c_suah_face · 연기: 반가움과 경계심 교차, 떨리는 눈동자
Cut #3 — 리버스 클로즈업 · 핸드헬드 · @p1_c_jihoon_face · 연기: 입을 열려다 삼키고 시선을 떨궜다 다시 든다
Cut #4 — 미디엄 투샷 · 좌→우 아크 무빙 · @p1_c_suah_full 과 @p1_c_jihoon_full 마주 섬 · 연기: 동시에 한 걸음 다가서려다 멈칫
Cut #5 — 인서트→틸트업 · 손에서 얼굴로 틸트 · 식은 커피잔 → @p1_c_suah_face · 연기: 옅은 미소, 안도
```
